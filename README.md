...
**如果使用了模块还卡，那么可以再将设备快连和设备互联两个傻逼系统应用毒瘤停用+冻结，坏处就是不能够使用呃，类似于互联的应用，让手机和电脑同步剪切板，或者说电脑访问手机的文件等等，这种厂家定制级别的互联软件，推测问题应该是他这个应该是启动了某些后台的一个进程，或者说服务它会一直监控剪切板的更改，目前不知道用什么手段实现的，也没有精力去深究了，模块理论上来讲，已经是把这些接口都去除了，但是他还是能够监听到剪贴板的内容，并且进行一个劫持和处理然后就导致了，每一次复制都会有非常大的性能开销，如果你使用了模块以后，不再会出现很卡的状况了，那不必禁用**
